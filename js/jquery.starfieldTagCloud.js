	...
	jQuery.fn.starfieldTagCloud = function(options)
		{
			// PRIVATE VARIABLES
			var defaults = {
					interval:100,             // interval between animation frames
					...
					hoverGravityFactor:0,   // gravity amount when user hovers over the tag cloud - works a multiplier of opts.gravity
			...
			var starField3D = function(scene, camera, width, height)
				{
					...
					renderParticles();
					y_timer = setInterval(renderParticles, opts.interval);


				// 此处暴露给window一个timer 修复bug 2019.07.30
				window.y_timer = y_timer
				};
