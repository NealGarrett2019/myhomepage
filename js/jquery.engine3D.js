...
;(function(jQuery) {
  ...
  //Create 2d Points
  jQuery.engine3D.displayObject3D.prototype.make2DPoint = function(
    x,
    y,
    depth,
    scaleFactor
  ...
  jQuery.engine3D.scene3D.prototype.Transform3DPointsTo2DPoints = function(
    points,
    axisRotations,
    camera
  ) {
    ...
    while (i--) {
      ...
      TransformedPointsArray[i] = displayObject.make2DPoint(
        x,
        y,
        -z,
        scaleFactor
  ...
  jQuery.engine3D.scene3D.prototype.renderCamera = function(camera) {
    for (var i = 0; i < this.sceneItems.length; i++) {
      ...
      var screenPoints = this.Transform3DPointsTo2DPoints(
        obj.pointsArray,
        jQuery.engine3D.axisRotation,
        camera
      )
      ...
      this.sceneItems[i].container.find('>div>ul>li').each(function(k) {
        ...
        jItemWrapper.css({
          ...
          opacity: screenPoints[k].scaleFactor - 0.5
        })
  ...
  //Center for rotation
  jQuery.engine3D.axisRotation = new jQuery.engine3D.displayObject3D().make3DPoint(
    0,
    0,
    0
  )
